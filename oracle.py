...
Operates strictly on the closed-form revenue model in
``synthetic_shops.analytical_revenue``. The GA's simulator-backed
fitness function in ``viz_ga._ga_compute_layout_score`` is structurally
independent — agent paths, heatmap, queue, abandonment — so any
ranking agreement between the two is informative, not circular.

Anti-circularity guarantee: **this module imports neither viz_ga nor
viz_optimize nor simulation**. The only project dependency is
``synthetic_shops``. Verified by ``test_oracle_anti_circularity()``.
"""
