...
These shops do NOT replace the dataset-loaded shop produced by
``dataset_layout.build_layout_from_calibration``. They are deliberate
small, controlled instances used solely for the synthetic-ground-truth
validation pipeline: generate -> solve analytically (oracle) -> run GA
on simulator-fitness landscape -> compare GA result against oracle.

The ``oracle.py`` module operates strictly on these dataclasses and the
closed-form revenue model defined here in ``analytical_revenue``. The
simulator-backed fitness function in ``viz_ga._ga_compute_layout_score``
is structurally independent (operates on agent paths, heatmap, etc.),
so any agreement between the two represents real evidence that the
geometric scoring captures the same placement principles as the
analytical revenue model.
"""
