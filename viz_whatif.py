...
class WhatIfMixin:
    ...
    def _snapshot_layout(self):
        return {
            ...
            'params': self._extract_simulation_parameters()
                      if self.customer_simulation.analytics.get('total_customers', 0) >= 5
                      else None,
            'timestamp': time.time(),
        }
    ...
    def _ab_layout_score(self, snap):
        ...
        try:
            ...
            score, _ = self._ga_compute_layout_score(positions, item_names, snap['params'])
            return score
        finally:
