...
class GAMixin:
    ...
    def _ga_compute_layout_score(self, positions, item_names, base_params):
        ...
        # Resolve each floor's heat array AND its max ONCE per call. The
        # previous code recomputed ``np.max(heat)`` (a full-grid reduction
        # over ~300k cells) twice for every item in both the traffic and
        # revenue-placement loops -- the dominant cost of the GA fast-fitness
        # on large shops.
        _heat_cache: dict = {}

        def _heat_for(fid):
            if fid in _heat_cache:
                return _heat_cache[fid]
            heat = floor_heat.get(fid)
            if heat is None and fid == getattr(self, 'current_floor', 1):
                heat = getattr(sim, 'heat_map_data', None)
            if (heat is None or not hasattr(heat, 'size') or heat.size == 0):
                entry = (None, 0.0)
            else:
                entry = (heat, float(np.max(heat)))
            _heat_cache[fid] = entry
            return entry

        for i, n in enumerate(item_names):
            heat, hmax = _heat_for(item_floor[n])
            if heat is None or hmax <= 0:
                continue
            cx, cy = item_center(i, n)
            xi = max(0, min(int(cx * res), heat.shape[0] - 1))
            yi = max(0, min(int(cy * res), heat.shape[1] - 1))
            traffic_score += float(heat[xi, yi]) / max(hmax, 1e-6)
            traffic_count += 1
        ...
        cross_score = 0.0
        cross_weight = 0.0
        for pair, count in dict(_cval(A, 'cross_merchandising', {}) or {}).items():
        ...
        if entrance_pos and checkout_pos:
            top_items = sorted((_cval(A, 'popular_items', {}) or {}).items(), key=lambda kv: kv[1], reverse=True)[:5]
        ...
        # Vectorized AABB overlap. Semantics are bit-identical to the prior
        # nested-Python-loop version (out-of-bounds = +1.0 per item;
        # same-floor item-item overlap = +1.0 per unordered pair;
        # item-vs-non-Section-wall overlap = +0.5 per pair) but ~30x faster
        # on UCI-sized shops (~100 items), which the GA fast-fitness calls
        # pop x gens times during the Optimize pipeline.
        sizes = np.array([item_data[n].get('size', (0.0, 0.0)) for n in item_names],
                         dtype=np.float64)
        px = positions[:, 0]; py = positions[:, 1]
        pw = sizes[:, 0];     ph = sizes[:, 1]
        floors_arr = np.array([item_floor[n] for n in item_names])

        # Out-of-bounds: +1.0 per offending item.
        oob = (px < 0) | (py < 0) | (px + pw > self.width) | (py + ph > self.height)
        overlap_penalty = float(oob.sum())

        # Item-item overlap (same floor, unordered pairs i<j): +1.0 each.
        no_ov = ((px[:, None] + pw[:, None] <= px[None, :]) |
                 (px[:, None] >= px[None, :] + pw[None, :]) |
                 (py[:, None] + ph[:, None] <= py[None, :]) |
                 (py[:, None] >= py[None, :] + ph[None, :]))
        same_floor = floors_arr[:, None] == floors_arr[None, :]
        ov_mat = (~no_ov) & same_floor
        overlap_penalty += float(np.triu(ov_mat, k=1).sum())

        # Item-vs-wall overlap (non-Section walls, per floor): +0.5 each.
        for fl in set(floors_arr.tolist()):
            m = floors_arr == fl
            if not m.any():
                continue
            xm, ym, wm, hm = px[m], py[m], pw[m], ph[m]
            for wname, wd in self.floors.get(fl, {}).get('walls', {}).items():
                if wname.startswith('Section_'):
                    continue
                wx, wy = wd['position']; ww, wh = wd['size']
                no_w = ((xm + wm <= wx) | (xm >= wx + ww) |
                        (ym + hm <= wy) | (ym >= wy + wh))
                overlap_penalty += 0.5 * float((~no_w).sum())

        # -- 6. REVENUE-WEIGHTED PLACEMENT -------------------------
        revenue_placement_score = 0.0
        conv_rates = dict(_cval(A, 'item_conversion_rates', {}) or {})
        ...
        for i, n in enumerate(item_names):
            ...
            xi = max(0, min(int(cx * res), heat.shape[0] - 1))
            yi = max(0, min(int(cy * res), heat.shape[1] - 1))
            revenue_placement_score += (item_revenues[i] / max_rev) * (float(heat[xi, yi]) / max(hmax, 1e-6))
            rev_count += 1
