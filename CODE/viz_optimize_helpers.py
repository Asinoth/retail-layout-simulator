...
class OptimizeHelpersMixin:
    ...
    def _analyze_current_performance(self):
        ...
        # Calibration-preferring reads: when a dataset is loaded these come
        # from analytics['calibration']; otherwise from the live counters.
        popular_items_src = _cval(analytics, 'popular_items', {}) or {}
        conv_rates_src    = _cval(analytics, 'item_conversion_rates', {}) or {}
        rev_by_area_src   = _cval(analytics, 'revenue_by_area', {}) or {}
        for item_name, item_data in items_catalog.items():
            source_name = item_data.get('source_name', item_name)
            score = 0

            # Popularity score (visits)
            popularity = max(popular_items_src.get(item_name, 0),
                             popular_items_src.get(source_name, 0))
            score += popularity * 2

            # Conversion rate score
            conversion_data = conv_rates_src.get(
                item_name,
                conv_rates_src.get(source_name, {'visits': 0, 'purchases': 0})
            )
            ...
            if item_category in rev_by_area_src:
                revenue_contribution = rev_by_area_src[item_category]
    ...
    def _implement_cross_merchandising(self):
        ...
        top_combos = sorted(
            (_cval(analytics, 'cross_merchandising', {}) or {}).items(),
            key=lambda x: x[1],
            reverse=True
        )[:3]
    ...
    def _optimize_customer_journey(self, traffic_zones):
        ...
        popular_items = sorted(
            (_cval(analytics, 'popular_items', {}) or {}).items(),
    ...
    def _optimize_checkout_area(self, performance_data):
        ...
        impulse_stats = []
        rates = _cval(self.customer_simulation.analytics,
                      'item_conversion_rates', {}) or {}
    ...
    def _find_safe_position_in_section(self, item_name, target_x, target_y):
        ...
        for radius in [0.3, 0.6, 1.0, 1.5, 2.0]:
            for angle in range(0, 360, 30):  # every 30deg
