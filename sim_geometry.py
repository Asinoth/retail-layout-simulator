...
class GeometryMixin:
    """Geometry caches, wall bins, and heatmap helpers for CustomerFlowSimulation.
    Extracted from simulation.py to keep each module under ~600 lines. Behavior is unchanged."""
    ...
    def _rebuild_geometry_caches(self):
        ...
        for fid, fdata in floors.items():
            ...
            for nm, data in walls.items():
                if nm.startswith('Section_') or nm in ('Checkout', 'WC'):
                    continue
