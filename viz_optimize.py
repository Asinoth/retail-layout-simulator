...
from sim_calibration import transient_occupancy_distribution
...
class OptimizeMixin:
    ...
    def _optimize_layout(self):
        ...
        if analytics.get('total_customers', 0) < 5:
    ...
    def _on_pre_window_end_inner(self):
        ...
        def _fast_fitness(chromosome):
            ...
            # Analytic expected 30-day revenue (matches _mc_engine mean structure)
            exp_converters = base_daily_cust * final_conv
            base_rev  = exp_converters * rev_base
            bsk_rev   = exp_converters * bsk_adj
            imp_rev   = exp_converters * imp_adj * impv_base
            return (base_rev + bsk_rev + imp_rev) * 30.0
        ...
        pipe['ga_mc_iters']  = ga_mc_iters
        pipe['ga_mc_days']   = ga_mc_days
        ...
        for fid, fdata in self.floors.items():
            if 'Checkout' in fdata.get('items', {}):
                cp = fdata['items']['Checkout']['position']
                cs = fdata['items']['Checkout']['size']
                checkout_pos = (cp[0] + cs[0] / 2, cp[1] + cs[1] / 2)
                checkout_floor = fid
                break
    ...
    def _finalize_optimization_metrics(self):
        ...
        for label, snap_key in [('A', 'pre_snapshot'), ('B', 'post_snapshot')]:
            ...
            # footing as the GA fitness function.
            from retail_literature import (
